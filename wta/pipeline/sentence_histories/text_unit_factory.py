...
class TextUnitFactory:
    ...
    def run(self, text, revision_id, ts, prev_tpsf):
        ...
        pre_tus, changed_tus, post_tus = self.detect_tus_diffs(merged_textunit_list, ts, prev_tpsf)
        print(f'TS: {ts.label.upper()} ({ts.startpos}-{ts.endpos}): |{ts.text}|')
        print(f'TEXT: {text}')
        print(f'INITIAL SENTENCE LIST: {sentence_list}')
        corrected_changed_tus = self.improve_segmentation_with_post_tus(changed_tus, post_tus)
        tus_states = self.collect_tus_states(pre_tus, corrected_changed_tus, post_tus, ts)
        corrected_textunits = [
            *pre_tus,
            *corrected_changed_tus,
            *post_tus
        ]
        print(f'FINAL TU LIST: {[(type(tu).__name__, tu.state.upper(), tu.text) for tu in corrected_textunits]}')
        return corrected_textunits, tus_states

    def split_in_textunits(self, sentence_list):
        textunit_list = []
        for s in sentence_list:
            contains_only_ws = True if re.search(RegularExpressions.ONLY_WS_RE, s) is not None else False
            ...
            if sen_wo_trailing_ws == '»':
                merged_tu_text = textunit_list[len(textunit_list)-1].text + sen_wo_trailing_ws
                merged_tu = textunit_list[len(textunit_list)-1].__class__(merged_tu_text)
                textunit_list[len(textunit_list)-1] = merged_tu
                sin_text = None if re.search(RegularExpressions.TRAILING_WS_RE, s) is None else re.search(RegularExpressions.TRAILING_WS_RE, s).group(0)
            ...
            if re.search(RegularExpressions.INITIAL_WS_RE, s) is not None:
                ...
                textunit_list.append(sin)
            sen_with_ws_trimmed = re.sub(RegularExpressions.INITIAL_WS_RE, '', sen_wo_trailing_ws)
    ...
    def merge_double_textunits(self, textunit_list):
        doubles = [(i, j) for i, j in zip_longest(textunit_list, textunit_list[1:]) if type(i).__name__ == type(j).__name__ != 'Sen']
        if len(doubles) == 0:
            ...
            for i, j in zip_longest(textunit_list, textunit_list[1:]):
                if type(i).__name__ == type(j).__name__ != 'Sen':
                    ...
                    text_wo_trailing_ws = re.sub(RegularExpressions.TRAILING_WS_RE, '', merged_tu_text)
    ...
    def detect_tus_diffs(self, textunits, ts, prev_tpsf):
        if ts.label in [TSLabels.MID, TSLabels.DEL, TSLabels.REPL]:
            pre_tus, changed_tus, post_tus = self.check_what_changed(prev_tpsf.textunits, ts)
            # if the edit consists in deleting or reducing sentence interspace
            if len(changed_tus) == 1 and type(changed_tus[0]).__name__ == 'Sin' and ts.label in [TSLabels.MID, TSLabels.DEL]:
                reduced_sin_content = re.sub(ts.text, '', changed_tus[0].text, count=1)
                if reduced_sin_content == '':
                    changed_tus = []
                elif re.search(RegularExpressions.ONLY_WS_RE, reduced_sin_content) is not None:
                    sin_content = re.search(RegularExpressions.ONLY_WS_RE, reduced_sin_content).group(0)
                    changed_tus = [Sin(sin_content)]
            else:
                changed_tus = [tu for tu in textunits if tu.text not in [*[prtu.text for prtu in pre_tus], *[potu.text for potu in post_tus]]]
    ...
    def improve_segmentation_with_post_tus(self, changed_tus, post_tus):
        ...
        for ctu in changed_tus:
            matching_sens = [psen for psen in post_sens if re.search(psen.text, ctu.text) is not None]
            if len(matching_sens) == 1:
                ...
                if not uppercase_letter or not end_punctuation:
                    ...
                    sen = Sen(re.sub(RegularExpressions.TRAILING_WS_RE, '', new_ctu_text))
                    corrected_changed_tus.append(sen)
                    sin_content = None if re.search(RegularExpressions.TRAILING_WS_RE, new_ctu_text) is None else re.search(
                        RegularExpressions.TRAILING_WS_RE, new_ctu_text).group(0)
                ...
                print(f'ATTENTION: {len(matching_sens)} textunits have been found within the textunit {ctu.text}')
        ...
        for tu in [*pre_tus, *changed_tus, *post_tus]:
            if len(tu.text) > 0:
                ...
                if tu in pre_tus:
                    ...
                    if ts.label in [TSLabels.INS, TSLabels.APP,
                                    TSLabels.PAST] and tu_starts_within_ts and tu_ends_within_ts:
                        tus_states.append(SenLabels.NEW)
                        tu.set_state(SenLabels.NEW)
                    else:
                        tus_states.append(SenLabels.MOD)
                        tu.set_state(SenLabels.MOD)
            currentpos = currentpos + len(tu.text)  # it's the index of the beginning of the next tu
        return tus_states
