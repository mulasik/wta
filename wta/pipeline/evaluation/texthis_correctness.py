...
def check_texthis_correctness(
    ...
    if text.strip() == last_tpsf.text.strip():
        print(
            "INFO: Successfully generated a text history. Last text version is the same as the text produced."
        )
        return True
    diffs = retrieve_mismatch_ranges(text.strip(), last_tpsf.text.strip())
    print(
        "INFO: Failure when generating text history. Last text version is not the same as the text produced."
    )
    ...
    if False not in empty_diffs:
        print(
            "INFO: Successfully generated a text history. The difference to original contains only whitespaces."
        )
        return True
    return False
