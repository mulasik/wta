...
# SIN
S_RE = r"[^\S^\n^\t^\r^\v]+"
PRECEDING_S_RE = re.compile(rf"\A{S_RE}")
TRAILING_S_RE = re.compile(rf"{S_RE}\Z")
ONLY_S_RE = re.compile(rf"\A{S_RE}\Z")

# PIN
PWS_RE = r"((\n+\t*)+|(\r+\n*\t*)+|(\t+\r*\n*)+|(\v+\r*\n*)+)"  # paragraph whitespace characters (pws): \r - carriage return, \n - newline, \t - tab, \v - vertical tab
PRECEDING_PWS_RE = re.compile(rf"\A{PWS_RE}")
TRAILING_PWS_RE = re.compile(rf"{PWS_RE}\Z")
ONLY_PWS_RE = re.compile(rf"\A{PWS_RE}\Z")
