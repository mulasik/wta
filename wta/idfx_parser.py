...
class IdfxParser:
    ...
    def __init__(self, idfx, config, nlp_model) -> None:
        ...
        self.all_tpsfs_ecm = (
            []
        )  # accumulates the revisions of TPSF based on edit operations
        self.all_tpsfs_pcm = (
            []
        )  # accumulates the revisions of TPSF based on pause duration
    ...
    def run(self) -> None:
        ...
        prev_endtime = int(
            [
                e.value.get_text()
                for e in entries
                if e.key.get_text() == "__LogRelativeCreationDate"
            ][0]
        )
        ...
        for event in progress:
            ...
            if event["type"] == "keyboard" and len(parts) == 2:
                ...
                if (
                    pos < prev_pos and prev_key not in [self.BACKSPACE, self.DELETE]
                ) or (
                    pos == prev_pos
                    and inserted_sequence != ""
                    and prev_key not in [self.BACKSPACE, self.DELETE]
                ):
                    ...
                    prev_tpsf = (
                        None if len(self.all_tpsfs_ecm) == 0 else self.all_tpsfs_ecm[-1]
                    )
                    tpsf = TpsfEcm(
        ...
        for tpsf in progress:
            if tpsf.relevance is True:
                ...
                if tpsf.transforming_sequence is not None:
                    irrelevant_ts_aggregated.append(
                        (
                            tpsf.transforming_sequence.text,
                            tpsf.transforming_sequence.label,
                        )
                    )
